...
/* 
 *	Evaluate the fitness of the computed feedforward Neural Network using 
 *	  Predicted_Output() and weighted fitness objectives.  
 */ 
{
...
/* 	
 *	Invert RMSE for minimizing it.
 *	If the RMSE is 0.0 then set it to something very large.  
 */ 
 	if(EQUAL(rmse, 0.0))
 		perform->accuracy = 10000.0;
 	else
 		perform->accuracy = 1.0/rmse;

/*
 *	Set fitness as a linear combination of the possible objectives.
 */
	fitness =  fitness_weights[FO_ACCURACY] 		* log(perform->accuracy)  			+
	           fitness_weights[FO_P_AND_L] 			* perform->p_and_l  			+
	           fitness_weights[FO_AVG_PL] 			* perform->avg_pl  				+
	           fitness_weights[FO_SHARPS_RATIO] 	* perform->sharps_ratio  		+
	           fitness_weights[FO_AVG_PL_TO_MAX_DD] * perform->avg_pl_to_max_dd  	+
	           fitness_weights[FO_CUM_PL_PC] 		* perform->cum_pl_pc		  	+
	           fitness_weights[FO_ROR_PC] 			* perform->ror_pc;
	           
	return(fitness);

} /* End of Double Use_Fitness_Objective() */ 
