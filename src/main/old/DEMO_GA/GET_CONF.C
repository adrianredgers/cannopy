...
DataPairsPtr *train_price_data, DataPairsPtr *test_price_data, DataPairsPtr *results,
char *results_file, char *summary_file, char *ga_cfg_file, char *cfg_file,
Double *init_capital, Double *transaction_size, Double *transaction_cost, Double *no_pos_zone)
/*
 ...
 */					
		if(!stricmp(new_word, "algorithm_config"))
		{
			...
			item = Scan_String(fp, ga_cfg_file, file_name, item);
			item = Scan_String(fp, new_word, file_name, item);     
		} /* End of parse "ga_config ..." */			
 ...
 */
	if(ga_cfg_file[0] == '\0')
		strcpy(ga_cfg_file, "ga.cfg");    
 	Get_Trading_Params(ga_cfg_file, init_capital, transaction_size, transaction_cost, 
no_pos_zone);
...
*/
	...
	{
		if(window)
			num_train_points = Count_File_Data(train_file, input_fields) - ignore_train_points;
		else
	...
	{
		if(window)
			num_test_points = Count_File_Data(test_file, input_fields) - ignore_test_points;  
		else
...
ignore_train_points, train_price_file, FALSE);
	...
	*results = Con_DataPairs(num_test_points, input_fields, output_fields, num_fields, 0, ga_cfg_file, FALSE);
...
/*
 *	Open and close the results files to clear them and write a header.
 */
 
	if(!results_file[0])
		Make_Filename(results_file, file_name, TEST_FILE_SUFFIX);
	fp = Open_File(results_file, "w", "Get_Configuration()");
	fclose(fp); 

	Make_Filename(summary_file, results_file, SUMMARY_FILE_SUFFIX);
	fp = Open_File(results_file, "w", "Get_Configuration()");
	fclose(fp); 
...
Void Get_Trading_Params(char *ga_cfg_file, Double *init_capital, Double *transaction_size, 
Double *transaction_cost, Double *no_pos_zone)
{
    ...
    fp = Open_File(ga_cfg_file, "r", "Get_Trading()");
	
/*
 *	Default values of everything.
 */
	*init_capital = 100000.0;
	*transaction_size = 100000.0;
	*transaction_cost = 1000.0;
	*no_pos_zone = 0.002;
		

	item = Scan_String(fp, new_word, ga_cfg_file, 0);
	while(!feof(fp))
	{
 ...
 */					
		if(!stricmp(new_word, "/*"))
		{
			do
			{
				item = Scan_String(fp, new_word, ga_cfg_file, item);
			} while(stricmp(new_word, "*/"));
			item = Scan_String(fp, new_word, ga_cfg_file, item);
		} /* End of parse comments */			
 ...
 *
 */					
		else if(!stricmp(new_word, "transaction_size"))
		{
			item = Scan_Float(fp, &float_val, ga_cfg_file, item);   
			*transaction_size = float_val;
			if(*transaction_size < 0.0)
				Error_Exit("\nTransaction size out of range.",new_word,item, ga_cfg_file);			
			item = Scan_String(fp, new_word, ga_cfg_file, item);      
 ...
 *
 */					
		else if(!stricmp(new_word, "transaction_cost"))
		{
			item = Scan_Float(fp, &float_val, ga_cfg_file, item);   
			*transaction_cost = float_val;
			if(*transaction_cost < 0.0)
				Error_Exit("\nTransaction cost out of range.",new_word,item, ga_cfg_file);			
			item = Scan_String(fp, new_word, ga_cfg_file, item);      
 ...
 *
 */					
		else if(!stricmp(new_word, "init_capital"))
		{
			item = Scan_Float(fp, &float_val, ga_cfg_file, item);   
			*init_capital = float_val;
			if(*init_capital < 0.0)
				Error_Exit("\nInitial capital out of range.",new_word,item, ga_cfg_file);			
			item = Scan_String(fp, new_word, ga_cfg_file, item);      
 ...
 *
 */					
		else if(!stricmp(new_word, "no_pos_zone"))
		{
			item = Scan_Float(fp, &float_val, ga_cfg_file, item);   
			*no_pos_zone = float_val;
			if(*no_pos_zone < 0.0)
				Error_Exit("\nNo-position zone out of range.",new_word,item, ga_cfg_file);			
			item = Scan_String(fp, new_word, ga_cfg_file, item);      
