...
/*
 *	Return a slot from a biased roulette wheel.
 *	Slots are numbered from 0 to num_slots-1 
 *	The values in wheel should form a probability density function.
 *	I.E. all items should be more than or equal to 0, and their sum should equal 1.
 */
{
	Int slot;
	...
	do
	{
		slot = Random_Int(0, num_slots-1);	
	} while (wheel[slot] < ((Double) rand())/RAND_MAX);
	
	return(slot);

} /* End of Int Roulette_Wheel() */
...
/*
 *	Normalize the fitnesses so that they form a probability density function. 
 *	Actually it's not quite a p.d.f because there is a minimum probability
 *	  of being chosen and they may sum to more than 1.
 */
{
	Int org;
	Double sum, min, limit, x_limit, inv_sum;

	limit = 1.0/pop->pop_size;  
	x_limit = 0.5*limit;
/*
 *	Offset the sum by the negative minimum.
 *	If the sum is zero then every organism is equally fit.
 *	Otherwise scale the offset fitnesses so that they add to zero.
 */	
	if(pop->min_fitness < 0.0)
		min = pop->min_fitness;	
	else
      	min = 0.0;
      	
	sum = pop->sum_fitness - (pop->pop_size * min);
					
	if(EQUAL(sum, 0.0))   
	{
		for(org=0; org<pop->pop_size; org++)
		{
			pop->normal_fitness[org] = limit;
	...
	else
	{
		...
		for(org=0; org<pop->pop_size; org++)
		{
			pop->normal_fitness[org] = (pop->fitness[org]-min)*inv_sum; 
			if(pop->normal_fitness[org] < x_limit)
				pop->normal_fitness[org] = x_limit;
			pop->ordering[org] = org;
		} 
	}	

} /* End of Void Normalize_Fitnesses() */
...
} /* End of Void Reorder_Population() */
